var duk_data = [
    {
        klausimas: 'Ar yra aprangos kodas?',
        atsakymas: 'TAIP. Aprangos kodas – auksinės ir/arba smaragdinės žalios akcentas.'
    },
    {
        klausimas: 'Kur ir kada vyks ceremonija?',
        atsakymas: 'Tiek ceremonija, tiek šventė su programa vyks toje pačioje vietoje – sodyboje „Vila RA“. Kažkur papildomai važiuoti nereikės. Ceremonijos pradžia – 15 val.'
    },
    {
        klausimas: 'Kada galiu atvykti į ceremonijos ir šventės vietą?',
        atsakymas: 'Sodyba duris svečiams atvers nuo 12 val. Tad tikrai bus galima atvykti anksčiau, susirasti savo nakvynės vietą bei užbaigti pasiruošimą šventei jau pačioje sodyboje (pvz., persirengti).'
    },
    {
        klausimas: 'Ar bus galimybė pernakvoti?',
        atsakymas: 'TAIP. Svečiams sodyboje tikrai atsiras vietos nakvynei. Sodyba siūlo miegamuosius su atskirais vonios kambariais bei kelis atskirus namelius. Labai prašome užpildyti nakvynės poreikį registracijos formoje, kad mes jums galėtume parinkti vietą, o sodybos šeimininkai galėtų pasirūpinti visais reikiamais patogumais.'
    },
    {
        klausimas: 'Ar bus parkingo vieta?',
        atsakymas: 'TAIP. Visi svečiai savo automobilius galės pastatyti šalia sodybos esančioje automobilių stovėjimo aikštelėje.'
    },
    {
        klausimas: 'Ką daryti,&nbsp;&nbsp;jei negalėsiu dalyvauti šventėje?',
        atsakymas: 'Jei dalyvauti negalėsite, labai prašome tai pažymėti registracijos formoje.'
    },
    {
        klausimas: 'Iki kada turiu pranešti apie dalyvavimą/nedalyvavimą?',
        atsakymas: 'Registracijos formą, kurioje galite pranešti apie savo dalyvavimą/nedalyvavimą, prašome užpildyti iki balandžio 8 d. Jeigu vėliau dėl pasikeitusių aplinkybių nebegalėsite dalyvauti šventėje, prašysime pranešti mums atskirai telefonu ar asmenine žinute per messenger.'
    },
    {
        klausimas: 'Ar šventėje bus vaikų?',
        atsakymas: 'TAIP. Į mūsų vestuvių šventę vaikai kviečiami, tad jų tikrai bus. Pastaba atvykstantiems su vaikais: dėl riboto miegamųjų vietų skaičiaus, prašome atvykti su vaikais, kurie yra įtraukti į jūsų kvietimą.'
    },
    {
        klausimas: 'Ar galėsiu fotografuoti/filmuoti?',
        atsakymas: 'TAIP. Filmuoti ir fotografuoti bus galima, bet labai kviečiame pasimėgauti švente padėjus telefonus į šoną :) Nuotraukomis pasirūpins profesionali fotografė tiek per ceremoniją, tiek šventės metu iki pat vidurnakčio, tad nuotraukuose tikrai rasite ir save.'
    },
    {
        klausimas: 'Kaip ir kada galėsiu gauti fotografės darytas nuotraukas?',
        atsakymas: 'Vestuvių nuotraukomis su visais svečiais pasidalinsime el. paštu ar kitais komunikacijos kanalais 3 mėn. bėgyje po įvykusios šventės.'
    }
];